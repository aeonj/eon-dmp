...
import eon.hg.fap.flow.command.impl.StartProcessInstanceCommand;
...
public class SubprocessNode extends Node {
	...
	private ProcessDefinition getTargetSubprocess(ProcessService processService) {
		...
		if(subprocessType.equals(SubprocessType.Id)){
			if(StrUtil.isEmpty(subprocessId)){
			...
			if(StrUtil.isEmpty(subprocessKey)){
			...
			if(StrUtil.isEmpty(subprocessName)){
